...
export function FileSendPage({
  ...
  return (
    <section className="dd-snaplink__workbench-page is-files" aria-label="文件发送">
      {header}
      <SendTargetSelector
      ...
      <div className="dd-snaplink__recent-sends">
        <div className="dd-snaplink__section-head">
          <span>
            <strong>最近发送</strong>
            <small>{recentFiles.length > 0 ? '最近的文件任务会显示在这里' : '还没有文件任务'}</small>
          </span>
        </div>
        {recentFiles.length > 0 ? (
          ...
          <EmptyState
            ...
            title="暂无最近发送"
            description={<span>选择文件后，任务会立即进入右侧传输队列。</span>}
          />
        )}
      </div>
    </section>
