...
TEST_FILE = Path(r'C:\Project\ddzhilian\tmp\receive-input-flow.txt')
TEST_FILE.write_text('input flow receive confirm test', encoding='utf-8')
result = {}
with sync_playwright() as p:
    ...
    # Select target device with the main card area, then trigger the workbench file input directly.
    ...
    a.screenshot(path=str(OUT / 'dd-receive-input-flow-a.png'), full_page=True)
    b.screenshot(path=str(OUT / 'dd-receive-input-flow-b.png'), full_page=True)
    browser.close()
    if not dialog_seen:
        raise SystemExit(1)
print(result)
