...
with sync_playwright() as p:
    ...
    a.wait_for_selector('.dd-snaplink__workbench-device', timeout=20000)
    ...
    dt = a.evaluate_handle("""() => {
      ...
      dataTransfer.items.add(new File(['receive dialog test'], 'receive-confirm-test.txt', { type: 'text/plain' }));
      return dataTransfer;
    }""")
    ...
    dialog_seen = False
    try:
        b.wait_for_selector('.dd-snaplink__receive-dialog', timeout=30000)
        dialog_seen = True
    except PlaywrightTimeoutError:
        pass
    ...
    result['b_receive_dialog_text'] = b.locator('.dd-snaplink__receive-dialog').inner_text(timeout=5000) if dialog_seen else ''
    result['b_pending_notice'] = b.locator('.dd-snaplink__receive-notice').inner_text(timeout=5000) if b.locator('.dd-snaplink__receive-notice').count() else ''
    a.screenshot(path=str(OUT / 'dd-receive-confirm-a.png'), full_page=True)
    b.screenshot(path=str(OUT / 'dd-receive-confirm-b.png'), full_page=True)
    browser.close()
    if not dialog_seen:
        raise SystemExit(1)
print(result)
