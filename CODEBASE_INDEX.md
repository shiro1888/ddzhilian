...
- Public room / room state / history / message and file recall: start at `use-ddzhilian.ts`, `server/src/index.ts`, `server/src/registry/room-registry.ts`, and `server/src/registry/history-registry.ts`; recall is still requested with a device history token, while admin-wide recall also requires a valid admin cookie.
