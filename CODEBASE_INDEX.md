...
- `src/app/components/AdminStage.tsx`: thin composition entry for the account-based admin console, using a light SnapLink-aligned default theme. Admin view state lives in `src/lib/use-admin.ts`, while the UI is split under `src/app/components/admin/` into login/sidebar/topbar, dashboard widgets, provider/model panels, online-device management, user/role management, toast, skeleton, and CSV-capable table components; SnapLink embeds this route without the old outer card frame. The login view exposes a dev-only one-click entry when the frontend runs in development mode.
