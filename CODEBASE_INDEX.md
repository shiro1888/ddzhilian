...
- `src/app/utils.ts`: small UI helpers plus rich-text/Markdown/code-block rendering, copyable code-block HTML generation, and code-first complete-HTML handling that creates the sandboxed preview iframe only when the fullscreen dialog is opened.

### Frontend feature components

- `src/app/components/SnapLinkStage.tsx`: primary lightweight interface for public-room creation/join, current-device display-name editing, file/text conversation with file recall actions, room-list navigation, pasted-image composer drafts that wait above the input before sending as inline chat images, a single room-header shared history menu for files/media/links, Beta custom theme controls for self/peer/AI message bubbles that submit color feedback to the backend, and top navigation between conversation, `/chat` AI chat, and `/image` image generation. `/admin` remains a direct route but is intentionally not exposed from the SnapLink top navigation.
...
- `src/app/components/AdminStage.tsx`: thin composition entry for the account-based admin console, using a light SnapLink-aligned default theme. Admin view state lives in `src/lib/use-admin.ts`, while the UI is split under `src/app/components/admin/` into login/sidebar/topbar, dashboard widgets, provider/model panels, online-device management, user/role/theme-feedback management, toast, skeleton, and CSV-capable table components; SnapLink embeds this route without the old outer card frame. The login view exposes a dev-only one-click entry when the frontend runs in development mode.
- `src/app/components/admin/`: split admin console components. High-value files are `DashboardGrid.tsx` for widget visibility/drag/sizing, `DashboardWidgets.tsx` for widget render mapping, `ProviderConfigPanel.tsx` plus manual/CLIProxy panels for OpenAI-compatible configuration, `OnlineDevices.tsx` for current WebSocket device list and display-name updates, `UserManagement.tsx` for searchable/sortable/paginated quota editing, `ThemeSubmissions.tsx` for SnapLink Beta color-feedback review/export, and `constants.ts` for admin formatting/model/dashboard helpers.
...
- `src/lib/use-admin.ts`: admin session, login/logout, dev-only one-click login, online-device polling refresh, AI settings save, history clear, Supabase user quota update, role add/delete, OpenAI-compatible model detection, API error parsing, and admin toast queue.
...
- `server/src/index.ts`: HTTP + WebSocket server entry, request handling, signaling flow, account auth endpoints, admin login/session/dev-only local entry/online-device snapshot endpoints, SnapLink Beta theme-submission collection, AI proxy endpoints, optional SearXNG search-before-AI prompt context, history download/upload/recall endpoints including admin-authorized text/file recall, image generation proxying with Supabase-backed quota reservation, concurrent upstream image request racing, free-plus-paid quota enforcement, and room recent-24h text context assembly for AI requests.
...
- `server/src/registry/history-registry.ts`: persisted history text/file metadata, text/file deletion, and cleanup.
...
- `server/src/registry/theme-submission-registry.ts`: SnapLink Beta custom-theme submission persistence, using Supabase when configured and local JSON as a development/fallback store.
...
- Primary conversation UI / room list / message area: start at `src/app/components/SnapLinkStage.tsx`.
- Device discovery / pairing / reconnect entry: start at `src/App.tsx`, `src/app/components/SnapLinkStage.tsx`, and `src/lib/use-ddzhilian.ts`; current-device rename uses `updateSettings({ deviceName })`, and legacy `/`, `/connect`, `/send`, `/receive`, and `/sessions` paths resolve to `/text`.
- File send / receive / progress: start at `src/App.tsx`, `src/lib/use-ddzhilian.ts`, and `src/app/components/SnapLinkStage.tsx`.
- Markdown / long text behavior: start at `SnapLinkStage.tsx`, `src/app/utils.ts`, and `use-ddzhilian.ts`; code-block formatting and copy-button HTML live in `src/app/utils.ts`, while click-to-copy/context actions live in SnapLink.
- Public room / room state / history / message and file recall: start at `use-ddzhilian.ts`, `server/src/index.ts`, `server/src/protocol.ts`, `server/src/registry/room-registry.ts`, and `server/src/registry/history-registry.ts`; recall is still requested with a device history token, while admin-wide recall also requires a valid admin cookie, and successful text/file recall emits `history-recalled` over WebSocket so other online room members immediately remove the recalled item; device rename continues through `update-settings` snapshots, with message/file display preferring the latest snapshot device name over stored historical labels.
...
- AI chat room context / `@Ai` image behavior: start at `src/App.tsx`, `src/lib/use-ddzhilian.ts`, and `server/src/index.ts`; room-scoped AI requests automatically prepend recent 24-hour room text context before sending to the model, `@Ai` can pass pasted or quoted inline chat images to OpenAI-compatible multimodal models, the parser still accepts legacy `@bot`, and non-public `bot-chat` rooms remain a backend capability for existing flows.
- AI image generation: start at `src/app/components/ImageAccountGate.tsx`, `src/app/components/ImageGenerationStage.tsx`, `src/lib/use-account-auth.ts`, `src/lib/use-ddzhilian.ts`, `server/src/index.ts`, `server/src/registry/account-registry.ts`, `server/src/registry/image-generation-history-registry.ts`, and `server/src/config.ts`; `/api/ai/image` requires an account cookie, creates an async job, accepts JSON text-to-image or multipart `image[]` edit inputs, uses `CODEX_IMAGE_MODEL` with default `gpt-image-2` and falls back through `CODEX_IMAGE_FALLBACK_MODELS` only when an upstream model is unavailable, reserves per-account free/paid quota in Supabase before calling the upstream image API, races `CODEX_IMAGE_PARALLEL_REQUESTS` concurrent upstream image requests for the same job, confirms quota after successful image/history persistence, releases quota on failures, converts upstream `b64_json` images into disk-backed `/api/ai/image/assets/:generationId/:index.png` URLs, returns/stores available per-image `byteSize`, `width`, and `height` metadata, `GET /api/ai/image/quota` returns free/paid/reserved/total quota, `GET /api/ai/image/jobs/:jobId` polls it, `GET /api/ai/image/history` supports `limit`, `beforeCreatedAt`, and `beforeGenerationId` cursor paging, and completed metadata persists to Supabase.
...
- Admin access / API key management: start at `src/lib/use-admin.ts`, `src/lib/use-admin-permissions.ts`, `src/app/components/AdminStage.tsx`, `src/app/components/admin/`, `server/src/index.ts`, `server/src/registry/account-registry.ts`, and `server/src/registry/admin-session-registry.ts`; `/admin` logs in with a Supabase Auth account, `ADMIN_SUPER_EMAILS` grants super-admin access, normal admins live in `admin_roles`, and the dev-only one-click entry uses `/api/admin/dev-login` only when the backend is clearly running in development mode. Role add/delete and `/api/admin/ai-config` require a super-admin session, `/api/admin/ai-config/detect` validates OpenAI-compatible API keys by reading `/models`, `/api/admin/permissions` exposes lightweight admin capabilities such as `canRecallAnyMessage`, `/api/admin/online-devices` returns the lightweight live online snapshot for polling, `/api/admin/online-devices/name` renames currently connected WebSocket devices and rebroadcasts snapshots, and non-super admin state responses strip raw API key values.
- SnapLink custom theme feedback: start at `src/app/components/SnapLinkStage.tsx`, `server/src/index.ts`, `server/src/registry/theme-submission-registry.ts`, `src/lib/use-admin.ts`, `src/app/components/admin/ThemeSubmissions.tsx`, and `supabase/migrations/20260522120000_snaplink_theme_submissions.sql`; the client stores the active theme locally, debounces Beta color submissions to `/api/snaplink/theme-submissions`, and Admin state includes the latest stored submissions for review/export.
